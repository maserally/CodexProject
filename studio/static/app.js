...
async function cloudWorkerAction(action){const status=$('#cloud-worker-status'),button=action==='test'?$('#test-cloud-worker'):(action==='bootstrap-accuracy'?$('#bootstrap-accuracy-worker'):$('#bootstrap-cloud-worker'));status.textContent=action==='test'?'正在测试 SSH 连接…':(action==='bootstrap-accuracy'?'正在并行下载模型、核对权重分片并生成 SHA-256 清单；无卡模式也可完成…':'正在安装基础环境；无卡模式会跳过 CUDA 验收…');button.disabled=true;try{const worker=cloudWorkerBody();worker.enabled=true;const data=await jsonFetch(`/api/cloud-worker/${action}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cloud_worker:worker})});if(action==='test'){status.textContent=!data.gpu||data.gpu==='not-found'?'SSH 正常 · 当前无 GPU（适合预上传和模型下载）':`SSH 与 GPU 正常 · ${data.gpu}`}else if(action==='bootstrap-accuracy'){status.textContent=data.gpu_verified?`最高精度模型下载、分片校验和 GPU 加载验收全部完成 · ${data.model_dir}`:`模型文件已完整下载并校验 · 当前无 GPU，开卡后会自动完成显存加载验收 · ${data.model_dir}`}else{status.textContent=data.gpu_verified?'云端基础环境安装完成，CUDA 验收通过':'云端基础环境安装完成 · 当前无 GPU，CUDA 验收将在开卡后自动执行'}await saveProviderSettings('cloud',false)}catch(e){status.textContent=`失败：${e.message}`}finally{button.disabled=false}}
