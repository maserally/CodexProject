...
function actionMarkup(j){if(['queued','running'].includes(j.status))return`<button onclick="pauseJob('${j.id}')">暂停并调整</button><button class="danger" onclick="cancelJob('${j.id}')">取消</button>`;if(j.status==='paused')return`<button class="danger" onclick="cancelJob('${j.id}')">取消任务</button>`;if(j.status==='staged')return`<button onclick="startStagedJob('${j.id}')">GPU 开机后开始处理</button><button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`;if(j.status==='failed'&&j.options.cloud_stage_only)return`<button onclick="retryStagedJob('${j.id}')">断点续传</button><button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`;if(j.status==='failed')return`<button onclick="retryFailedJob('${j.id}')">从失败处继续</button><button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`;return`<button class="danger" onclick="deleteJob('${j.id}')">删除任务</button>`}
